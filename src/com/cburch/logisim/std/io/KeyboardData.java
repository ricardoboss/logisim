...
class KeyboardData implements InstanceData, Cloneable {
	...
	public char dequeue() {
		...
		if (len == 0) return '\0';
		char ret = buf[0];
		if (len - 1 >= 0) System.arraycopy(buf, 1, buf, 0, len - 1);
		bufferLength = len - 1;
		int pos = cursorPos;
	...
	public boolean insert(char value) {
		...
		int pos = cursorPos;
		if (len - pos >= 0) System.arraycopy(buf, pos, buf, pos + 1, len - pos);
		buf[pos] = value;
		bufferLength = len + 1;
		cursorPos = pos + 1;
		str = null;
	...
	public boolean delete() {
		...
		if (len - pos + 1 >= 0) System.arraycopy(buf, pos + 1, buf, pos + 1 - 1, len - pos + 1);
		bufferLength = len - 1;
		str = null;
		dispValid = false;
		return true;
